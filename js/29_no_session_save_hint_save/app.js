...
class JLPTStudyApp {
  ...
  isWordDue(kanji, marking) {
    ...
    const ts = this.markingTimestamps[kanji];
    if (!ts) return true; // No timestamp = never reviewed = due
    const intervalDays = this.srsIntervals[marking] || 7;
    const markedDate = new Date(ts);
  ...
  getDueStats() {
    const stats = {};
    for (let k = 1; k <= 6; k++) {
      const wordsInCat = this.kanjiWords.filter(w => getMarking(this.markings, w) === k);
      const dueWords = wordsInCat.filter(w => this.isWordDue(w.kanji || w.raw, k));
      stats[k] = { total: wordsInCat.length, due: dueWords.length };
    }
    return stats;
  }
  ...
  startSRSTest() {
    ...
    if (this.srsConfig.selectionMode === 'marking') {
      ...
      // Check if SRS interval filtering is enabled
      const useSRSIntervals = this.srsConfig.useSRSIntervals !== false; // default true
      for (let k = 1; k <= 6; k++) {
        const count = this.srsConfig.markingCounts[k];
        if (count <= 0) continue;
        let pool = wordPool.filter(w => getMarking(this.markings, w) === k);
        // Filter to only due words if SRS intervals enabled
        if (useSRSIntervals) {
          pool = pool.filter(w => this.isWordDue(w.kanji || w.raw, k));
        }
        allWords.push(...sampleArray(pool, count));
      }
    } else {
  ...
  _showGroupPreview(groupId, anchorEl) {
    ...
    popup.innerHTML = `
      ...
      <div class="group-preview-popup__words">
        ${memberWords.map(w => `
          <div class="group-preview-popup__word">
            <span class="group-preview-popup__kanji">${escapeHtml(w.kanji)}</span>
            <span class="group-preview-popup__reading">${escapeHtml(w.hiragana || '')}</span>
            <span class="group-preview-popup__meaning">${escapeHtml(w.meaning_en || w.meaning || '')}</span>
          </div>
        `).join('')}
      </div>
    ...
    popup.querySelector('[data-goto-group]')?.addEventListener('click', () => {
      this._dismissGroupPreview();
      const gid = parseInt(popup.querySelector('[data-goto-group]').dataset.gotoGroup);
      const g = this.wordGroups.find(g => g.id === gid);
