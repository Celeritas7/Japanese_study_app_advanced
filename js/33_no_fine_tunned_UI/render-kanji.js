...
import { useTokens, getTokensC } from './tokens.js';
...
export function renderSentencePanel(app) {
  ...
  // Legacy stem/indexOf predicate — used VERBATIM when a sentence has no
  // tokens_json (Decision 7.4: do not improve the fallback).
  const legacyMatches = (s) => {
  ...
  // Phase 7 (Q1=B): when the sentence has valid Sudachi tokens, match
  // against Mode C long-compound tokens by surface OR lemma. This is where
  // Bug #11 dies — 会社を立てる no longer false-matches a sentence that
  // merely contains 会社, because the multi-group AND is now checked
  // against real tokens, not brittle stem indexOf.
  const matchesSentence = (s) => {
    if (linkedSentenceIds.has(s.id) || !s.sentence) return false;
    if (!useTokens(s)) return legacyMatches(s);

    const toks = getTokensC(s);
    const hit = (needle) =>
      !!needle && toks.some(t => t.s === needle || t.l === needle);

    // Whole dictionary form first (e.g. 立てる / 会社を立てる via lemma).
    if (hit(wordKanji)) return true;
    // Multi-group AND preserved: every kanji group must match some token.
    if (kanjiGroups.length >= 2) return kanjiGroups.every(hit);
    // Single-group safety net: stem against token surfaces (catches an
    // unexpected Sudachi split). Token-bounded, not sentence indexOf.
    if (kanjiStem && kanjiStem.length >= 2 && kanjiStem !== wordKanji) {
      return toks.some(t => t.s.indexOf(kanjiStem) !== -1);
    }
    return false;
  };
          ...
          ${linked.length > 0 ? `
            <div class="space-y-2">
              ${linked.map(item => {
                ...
                const tappableHtml = renderTappableSentence(item, wordKanji, knownKanjiSet);
            ...
            <div class="pt-3 border-t border-slate-700/50">
              ...
              <div class="space-y-1.5">
                ${unlinked.map(s => {
                  ...
                  const tappableUnlinked = renderTappableSentence(s, wordKanji, knownKanjiSet);
