...
export function attachEventListeners(app) {
  ...
  // Story search — IME-safe with debounce
  const storyInput = document.getElementById('storySearchInput');
  if (storyInput) {
    let composing = false;
    let debounceTimer = null;
    
    storyInput.addEventListener('compositionstart', () => { composing = true; });
    storyInput.addEventListener('compositionend', (e) => {
      composing = false;
      // Trigger search after IME confirms
      app.storyFilter = e.target.value;
      if (!e.target.value) app.storySearchMode = 'groups';
      clearTimeout(debounceTimer);
      debounceTimer = setTimeout(() => {
        app.render();
        const el = document.getElementById('storySearchInput');
        if (el) el.focus();
      }, 100);
    });
    
    storyInput.addEventListener('input', (e) => {
      if (composing) return; // Don't touch anything during IME
      app.storyFilter = e.target.value;
      if (!e.target.value) app.storySearchMode = 'groups';
      clearTimeout(debounceTimer);
      debounceTimer = setTimeout(() => {
