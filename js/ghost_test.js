...
let GH={topic:null, marks:{}, strokes:[], tool:'pen', wi:1, ghost:false, items:[], brush:0, time:{}, focus:null, act:0};
...
async function jpUpdateReviews(log, url, key){
  ...
  const rows=log.map(l=>{
    ...
    const row={kanji:l.kanji, last_review:iso, next_review:next, last_rating:l.rating, updated_at:iso, step:s.step, lapses:s.lapses,
      ...
      history:[...(Array.isArray(prev.history)?prev.history:[]), {d:l.test_date, r:l.rating, s:l.seconds||0}].slice(-20)};
